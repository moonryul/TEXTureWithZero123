...
class TEXTure:
    def __init__(self, cfg: TrainConfig):
        ...
        file_suffix = "use_diff_masks" if self.cfg.guide.use_diff_masks else "use_object_mask"
        file_suffix += "+max_z_normals_" + str(int(self.cfg.optim.alpha)) if self.cfg.optim.learn_max_z_normals else "+view_masks"
            
        file_suffix += "+sd_blend" if self.cfg.guide.sd_blending else "+no_sd_blend"
        file_suffix += "+zero123_blend" if self.cfg.guide.zero123plus_blending else "+no_zero123_blend" 
        file_suffix +=  "+sd_inpaint" if self.cfg.guide.sd_inpaint else "+no_sd_inpaint"
        file_suffix +=  "+zero123_inpaint" if self.cfg.guide.zero123plus_inpaint else "+no_zero123_inpaint"
              
        self.exp_path = make_path(self.cfg.log.exp_dir / file_suffix)  #MJ: the / operator can be used to concatenate paths and strings.
    ...
    def paint_zero123plus(self):
        ...
        zero123plus_prep_start_time = time.perf_counter()  
        ...
        rgb_output_front_gen, object_mask_front = self.paint_viewpoint(frontview_data, should_project_back=True)
        ...
        # Render with use-median = False (default), so that the default magenta color of the texture map will NOT be replaced
        # the mean color of the learned part   
        outputs_magenta = self.mesh_model.render(theta=self.thetas, phi=self.phis, radius=self.radii, background=background_gray)
        rgb_renders_magenta = outputs_magenta['image']
        rgb_output_front_magenta = rgb_renders_magenta[0:1]
        render_cache = outputs_magenta['render_cache']
        object_masks = outputs_magenta['mask']

        B, _, _, _ = object_masks.shape

        cropped_rgb_renders_list = self.get_cropped_rgb_renders(rgb_renders_magenta, object_masks)
        diff_masks_list = []
        ...
        for i in range(B):
            ...
            cropped_diff = (cropped_rgb_renders_list[i].detach() - torch.tensor(self.mesh_model.default_color).view(1, 3, 1, 1).to(
            ...
            #MJ: tile_size = 320 // self.zero123plus.vae_scale_factor #MJ: 320/8 = 40
            mask_latent = F.interpolate(
                 #MJ: dilated_cropped_mask,
                  cropped_generate_mask,
                (320, 320),
                mode='nearest'  #MJ: 320 // pipeline.vae_scale_factor = 40
            )
            diff_masks_list.append( mask_latent)
        ...
        outputs_median = self.mesh_model.render(
            ...
            use_median=True 
            #MJ: This render changes the texture map by replacing the default magenta part by the mean of the learned part, 
            # but it clones self.texture_img before updating, so it does not affect the learning of the texture map from the view images later on
        )
  
        rgb_renders_median = outputs_median['image']
        #MJ: cropped_rgb_renders_list = self.get_cropped_rgb_renders(rgb_renders_median, object_masks)

        # MJ: prepare the front view image
        rgb_output_front_median = rgb_renders_median[0:1]
   
        front_image_rgba = torch.cat((rgb_output_front_median, object_mask_front), dim=1)
        ...
        self.log_train_image(  rgb_output_front_gen, 'paint_zero123plus:front_image (generated)')
        self.log_train_image(  rgb_output_front_magenta, 'paint_zero123plus:front_image (magenta)')
        self.log_train_image(rgb_output_front_median, 'paint_zero123plus:front_image (median)')
        ...
        depth_TEXTure = outputs_median['depth']
        depth_zero123 = 1 -  depth_TEXTure
        ...
        depth_rgba = torch.cat((depth_zero123, depth_zero123, depth_zero123, object_masks), dim=1)
        depth_rgba_TEXTure = torch.cat((depth_TEXTure, depth_TEXTure, depth_TEXTure, object_masks), dim=1)
        ...
        cropped_rgb_depths_small_list_TEXTure = []
        ...
        for i in range(B):
            ...
            cropped_rgb_depths_small_list.append( F.interpolate(
                 cropped_depth_rgba, (320, 320),
                mode='bilinear',
                align_corners=False )
            )
            
            cropped_depth_rgba_TEXTure = crop(depth_rgba_TEXTure[i][None])
         
            cropped_rgb_depths_small_list_TEXTure.append( F.interpolate(
                 cropped_depth_rgba_TEXTure, (320, 320),
                mode='bilinear',
                align_corners=False )
            )
            
             
            cropped_rgb_render =  crop( rgb_renders_median[i][None])
            cropped_rgb_renders_small_list.append(F.interpolate(
                cropped_rgb_render, (320, 320),   
                mode='bilinear',
                align_corners=False
            ))
            
            cropped_mask = crop( object_masks[i][None])
            
            cropped_masks_list.append( F.interpolate(
                  cropped_mask, (320, 320),   
                mode='bilinear',
                align_corners=False
            ))
        ...
        cropped_depth_grid_TEXTure = torch.cat((
            torch.cat((cropped_rgb_depths_small_list_TEXTure[1], cropped_rgb_depths_small_list_TEXTure[4]), dim=3),
            torch.cat((cropped_rgb_depths_small_list_TEXTure[2], cropped_rgb_depths_small_list_TEXTure[5]), dim=3),
            torch.cat((cropped_rgb_depths_small_list_TEXTure[3], cropped_rgb_depths_small_list_TEXTure[6]), dim=3),
        ), dim=2)
        ...
        masks_grid_tensor  = torch.cat((
            torch.cat((cropped_masks_list[1], cropped_masks_list[4]), dim=3),
            torch.cat((cropped_masks_list[2], cropped_masks_list[5]), dim=3),
            torch.cat((cropped_masks_list[3], cropped_masks_list[6]), dim=3),
        ), dim=2) #MJ: masks_grid is the mask based on the diff map from the default magenta texture map
        
        
            
        diff_masks_grid_tensor = torch.cat((
            torch.cat((diff_masks_list[1], diff_masks_list[4]), dim=3),
            torch.cat((diff_masks_list[2], diff_masks_list[5]), dim=3),
            torch.cat((diff_masks_list[3], diff_masks_list[6]), dim=3),
        ), dim=2) #MJ: masks_grid is the mask based on the diff map from the default magenta texture map

        masks_grid_latent = F.interpolate(
                  masks_grid_tensor, (40, 40),   
                mode='bilinear',
                align_corners=False
            )
        diff_masks_grid_latent = F.interpolate(
                  diff_masks_grid_tensor, (40, 40),   
                mode='bilinear',
                align_corners=False
            )       
        ...
        cropped_masks_grid_rgb = torch.cat((masks_grid_latent, masks_grid_latent,masks_grid_latent), dim=1)
        diff_masks_grid_rgb = torch.cat((diff_masks_grid_latent, diff_masks_grid_latent,diff_masks_grid_latent), dim=1)
        self.log_train_image(cropped_depth_grid[:, 0:3], 'zero123plus-prep:cropped_depth_grid')
        self.log_train_image(cropped_depth_grid_TEXTure[:, 0:3], 'zero123plus-prep:depth_grid_TEXTure')
      
        self.log_train_image(cropped_masks_grid_rgb, 'zero123plus-prep:cropped_masks_grid')
        self.log_train_image(diff_masks_grid_rgb, 'zero123plus-prep:diff_masks_grid')
        self.log_train_image(cropped_rgb_renders_grid, 'zero123plus-prep:cropped_rgb_renders_grid (magenta boundary inbreaking??)')
        ...
        if  self.cfg.guide.use_diff_masks:
          masks_grid_latent =  diff_masks_grid_latent
                
        masked_input_latents =  gt_renders_latent_grid * (masks_grid_latent < 0.5) + 0.5 * (masks_grid_latent >= 0.5) 
        ...
        self.zero123plus.masked_input_latents = masked_input_latents.half()
        self.zero123plus.masks_grid = masks_grid_latent.half()
        ...
        def on_step_end(pipeline, iter, t, callback_kwargs):
            ...
            #Here, we decode and display the latents that is  being denoised for debugging purpose.
            
            grid_latent = callback_kwargs["latents"] # JA: grid_latent is z_{t-1}, that is, z_{t_prev}
            
            unscaled_latent_grid = unscale_latents(grid_latent)  #MJ: grid_latent = callback_kwargs["latents"]
            scaled_images =  pipeline.vae.decode(
                    unscaled_latent_grid.half() / pipeline.vae.config.scaling_factor,
                    return_dict=False
                )[0]  
            ...
            self.log_train_image(  postprocessed_images, f'on_step_end:denoised image-iter={iter}')    
                
                
            #MJ: check if we want to project each latent image back to learned the noisy texture map
            if self.cfg.guide.interleave_mode:    
            ...
            #MJ: are_equal( grid_latents_otherviews,   grid_latent_otherviews2) == True 
                ...
                unscaled_latent_otherviews = unscale_latents(grid_latent_otherviews)  #MJ: grid_latent = callback_kwargs["latents"]
                scaled_images =  pipeline.vae.decode(
                ...
                rgb_outputs_allviews = torch.cat( [rgb_output_front_median, rgb_outputs_full_size], dim=0)
                
                self.project_back_only_texture_atlas(   #When render_cache is not None, the camera pose is not needed
                     render_cache=self.render_cache, background=self.background_gray, rgb_output= rgb_outputs_allviews,
        ...
        result = self.zero123plus(
            ...
            callback_on_step_end=on_step_end #MJ: use the callback to display the decoded latents during denoising
        ).images[0]
        
        if not self.cfg.guide.interleave_mode:
            #MJ: self.zero123plus() return image = self.image_processor.postprocess(image, output_type=output_type, do_denormalize=do_denormalize)

            #MJ: result is already an image tensor whose rgb values range within (0,1)
            #MJ: grid_image = torchvision.transforms.functional.pil_to_tensor(result).to(self.device).float() / 255
            grid_image = result
            self.log_train_image(grid_image[None], f'zero123plus_after:result_grid_image_{self.cfg.optim.seed}')

            images = split_zero123plus_grid(grid_image, 320)
            rgb_outputs = []
            for i in range(B):
                if i == 0:
                    rgb_output = rgb_output_front_median
                else:
    ...
    def evaluate(self, dataloader: DataLoader, save_path: Path, save_as_video: bool = False): #MJ: dataloader=self.dataloaders['val']
        ...
        if save_as_video: #MJ: false in our experiment
            all_preds = []
        for i, data in enumerate(dataloader): # render for each viewpoint
    ...
    def paint_viewpoint(self, data: Dict[str, Any], should_project_back=True):
        ...
        #MJ: It is assumed that In ConTEXTure, we call paint_viewpoint() only for the front view point
        ...
        outputs_magenta = self.mesh_model.render(theta=theta, phi=phi, radius=radius, background=background)
        render_cache = outputs_magenta['render_cache'] # JA: All the render outputs have the shape of (1200, 1200)
        depth_render = outputs_magenta['depth']
        rgb_render_magenta = outputs_magenta['image']
        ...
        self.log_train_image( rgb_render_magenta, 'paint_viewpoint:rgb_render (with the default magenta tex)')
        ...
        update_mask = outputs_magenta['mask']
      
        # Crop to inner region based on object mask
        min_h, min_w, max_h, max_w = utils.get_nonzero_region_tuple( outputs_magenta['mask'][0, 0])
        crop = lambda x: x[:, :, min_h:max_h, min_w:max_w]
        cropped_rgb_render = crop( rgb_render_magenta) # JA: This is rendered image which is denoted as Q_0.
        ...
        self.log_train_image(cropped_rgb_render, name='paint_viewpoint:cropped_rgb_render(front)')
        self.log_train_image(cropped_depth_render.repeat_interleave(3, dim=1), name='paint_viewpoint:cropped_depth (front)')
        ...
        #self.diffusion.use_inpaint = self.cfg.guide.use_inpainting and self.paint_step > 1
         #MJ: In the case of the front viewpoint, do not use inpainting to generate it
        ...
        self.log_train_image(cropped_rgb_output, name='paint_viewpoint:cropped_rgb_output (img2img, magenta,front)')
        ...
        rgb_output = rgb_render_magenta.clone() # JA: rgb_render shape is 1200x1200
        ...
        self.log_train_image(rgb_output, name='paint_viewpoint:cropped_rgb_output (full_size,front)')

        # Project back
        object_mask = outputs_magenta['mask'] # JA: mask has a shape of 1200x1200
        ...
        if should_project_back:
            ...
            self.log_train_image(fitted_pred_rgb, name='paint_viewpoint:fitted_pred_rgb (magenta,front)')
    ...
    def project_back(self, render_cache: Dict[str, Any], background: Any, rgb_output: torch.Tensor,
        ...
        #MJ: use the object mask as the render update mask, not the masks obtained by blurring the boundary)
    ...
    def project_back_only_texture_atlas(self, render_cache: Dict[str, Any], background: Any, rgb_output: torch.Tensor,
        ...
        # #MJ: What is the purpose of this restriction of the render_update_mask??
        # if self.cfg.guide.strict_projection:
        #     blurred_render_update_mask[blurred_render_update_mask < 0.5] = 0
        ...
        # for i in range(rgb_output.shape[0]):
        #    self.log_train_image(rgb_output[i][None] * render_update_mask[i][None], f'project_back_texture_atlas_prep:project_back_input_{i}')  
    ...
    def project_back_max_z_normals(self):
        ...
        for v in range( len(self.thetas) ):  
            ...
            curr_max_z_normals_pred_v = curr_max_z_normals_pred_v *    render_update_mask_v.float()    
            max_z_normals_target_v = max_z_normals_target_v *   render_update_mask_v.float()    
