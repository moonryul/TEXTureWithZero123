...
class TEXTure:
    ...
    # Create the batch of viewpoints:
        ...
        self.rgb_output_front = self.output_frontview['image']
        ...
        self.rgb_render_raw_allviews = self.output_rendered_allviews['image']
        ...
        self.z_normals_cache_allviews = None
        ...
        self.depth_rgba_allviews = torch.cat((self.depth_allviews, self.depth_allviews, self.depth_allviews, self.mask_allviews), dim=1)
        ...
        cropped_front_image = crop(self.front_image)

        self.zero123plus_cond = pad_tensor_to_size(cropped_front_image, 1200, 1200, value=1) 
        # JA: pad the front view image with ones so that the resulting image will be 1200x1200. 
        # This makes the background white;                                                                                          # This is new code. cropped_front_image is the cropped version. In control zero123 pipeline, zero123_front_input is used without padding
        ...
        self.depth_grid = torch.cat((
            torch.cat((self.depth_rgba_allviews[1], self.depth_rgba_allviews[4]), dim=3),
        ...
        self.front_image_for_zero123plus  =  \
                           F.interpolate(self.clean_front_image, (320, 320), mode='nearest') #MJ: (1,3,1200,1200) => (1,3,320,320)
        
                            
        self.preprocessed_front_image  = self.zero123plus.image_processor.preprocess(self.front_image_for_zero123plus) 
        ...
        self.clean_front_image_latent = self.scale_latents( 
                        ...
                        self.scale_image( self.preprocessed_front_image.half()),
                        return_dict=False
        ...
        #MJ:  we use the object mask, i.e., outputs['mask'], as curr_mask: => Test it: outputs['mask']  will be larger than exact_generate_mask
        ...
        def on_step_end(pipeline, i, t, callback_kwargs): 
            ...
            grid_latent_otherviews_batch =  split_zero123plus_grid(grid_latent_otherviews, 320 //   pipeline.vae_scale_factor)  #MJ: 320/8 = 40; tensor => a list of  tensors
            ...
            #MJ: combine the grid_latent_otherviews and the noisified front latents; the are generated images, but not rendered ones    
            ...
            latent_frontview = blended_latents_allviews[0] #MJ: This is the noisy front latent
            latents_otherviews =  blended_latents_allviews[1:] 
            ...
            unscaled_front_latent = self.unscale_latents(latent_frontview)
            #In Zero123Plus, the images are also scaled for training. So, after denoising, they are unscaled
            decoded_unscaled_front_image = self.unscale_image(pipeline.vae.decode(unscaled_front_latent.half() / pipeline.vae.config.scaling_factor, return_dict=False)[0])
            #MJ: Tensor decoded_unscaled_image ranges over [0,1]
                      
            unscaled_latents_otherviews = self.unscale_latents(latents_otherviews)
            #In Zero123Plus, the images are also scaled for training. So, after denoising, they are unscaled
            decoded_unscaled_images_otherviews = self.unscale_image(pipeline.vae.decode(unscaled_latents_otherviews.half() / pipeline.vae.config.scaling_factor, return_dict=False)[0])
            #MJ: Tensor decoded_unscaled_image ranges over [0,1]
            ...
            decoded_list_unscaled_image = split_zero123plus_grid( decoded_unscaled_images_otherviews , 320) #MJ: decoded_grid_image is tensor; images is a list of list of tensors

            decoded_unscaled_images = torch.cat( decoded_unscaled_front_image +  decoded_unscaled_images_otherviews  )
            #MJ: Change the shape of the decoded image to the full size (1200,1200) of the rendered image
            #MJ: image =(1,3,1200,1200)
            rgb_outputs_noisy = F.interpolate(decoded_unscaled_images, (1200, 1200), mode='bilinear', align_corners=False) #MJ: we can transform it as a batch mode
            ...
            for viewpoint_num in range( rgb_outputs_noisy.shape[0]):
               self.log_train_image(rgb_outputs_noisy[viewpoint_num], f'rgb_outputs_denoising{i}_viewpoint{viewpoint_num}')
            ...
            self.project_back_only_texture_atlas(iter=i, #MJ: denoising iteration
                render_cache=None, background=background, rgb_output=rgb_outputs_noisy,
                object_mask=object_mask, update_mask=object_mask, z_normals=z_normals, z_normals_cache=None
            )
            
                       
            #MJ: The blended_latents except for the first one needs to be arranged to the grid_latent for the next denoising step
            callback_kwargs["latents"] = torch.cat((
                ...
                torch.cat((blended_latents[3], blended_latents[6]), dim=3),
            ), dim=2).half()
