...
class TEXTure:
    ...
    def define_view_weights(self):
        ...
        if not self.cfg.optim.learn_max_z_normals: #MJ: when you do not use learn_max_z_normals
            ...
            for v in range( len(self.thetas) ):                
               self.log_train_image(torch.cat([max_z_normals[v][None], max_z_normals[v][None],max_z_normals[v][None]],
                                              dim=1), f'max_z_normals[{v}]')
    ...
    def paint_zero123plus(self):
        ...
        self.background_gray = torch.Tensor([0.5, 0.5, 0.5]).to(self.device)
        ...
        front_image = rgb_output_front 
        #MJ: In the first attempt, rgb_output_front had the magenta default color of the texture along the boundary of the rendered front view image
        # This affects badly  the project_back_only_texture_atlas which learns the texture map from the generated view images.
        #MJ: MJ modified rgb_output_front a little bit within project_back() 
        # by changing  the default magenta region of the texture atlas to the median color of the learned texture map
        # and re-rendering the updated texture map
        
         
        #MJ: At this point, self.texture_img has been learned from the front view image; 
        # In this learnig precess, the default magenta part of the texture map has been NOT changed to the meidan color of the learned texture, 
        # This occurs only for the views after the first one
        # When the mesh is rendered hereafter  this learned texture map will be used.
        ...
        # background black, but we would like to use a white background: 
        ...
        outputs_with_magenta_map = self.mesh_model.render(theta=self.thetas, phi=self.phis, radius=self.radii, background=self.background_gray)
        self.object_mask = outputs_with_magenta_map['mask']       
        ...
        #MJ: Create the object masks for each view based on the diff masks.
        ...
        for i in range( len(self.thetas) ):
            ...
            cropped_diff = (  self.cropped_rgb_renders_with_magenta_map_list[i].detach() - torch.tensor(self.mesh_model.default_color).view(1, 3, 1, 1).to(
            ...
            diff_batch = cropped_diff.unsqueeze(0)
            ...
            self.log_train_image( torch.cat( [cropped_generate_mask,cropped_generate_mask,cropped_generate_mask], dim=1),  f'v=cropped_generate_mask-{i}:')        
            ...
            curr_mask =   mask_latent
            self.log_train_image( torch.cat( [curr_mask,curr_mask,curr_mask], dim=1),  f'zero123++prep:curr_mask-v={i}') 
        #End for i in range( len(self.thetas) ) 
        
        self.mask_allviews_latent = torch.cat( masks_latent_list, dim=0)   
        
        #MJ: use_median = True means that the mesh is rendered using the texture map
           # where the default magenta part is filled by  the median color of the learned texture map; 
           # So the magenta part is replaced by the median color of the learned part of the texture map
           # ==> But note that we also render the mesh with use-median=True to create the diff-based object masks for each view
                          
        self.outputs_with_median_map = self.mesh_model.render(theta=self.thetas, phi=self.phis, radius=self.radii,
                                            background=self.background_gray, use_median=True)
        ...
        depth = 1 -  self.outputs_with_median_map['depth']
        ...
        #self.object_mask =  self.outputs_with_median_map['mask']
       ...
       #cropped_depth_rgba = utils.crop_img_to_bounding_box( depth_rgba, self.bounding_boxes)
        ...
        self.rgb_renders =  self.outputs_with_median_map['image']
        self.render_cache =  self.outputs_with_median_map['render_cache']
       ...
       # but got input with spatial dimensions of [3, 886, 886] and output size of (320, 320)               
        ...
        scaled_rgb_renders_small =  scale_image(preprocessed_rgb_renders_small.half() ) #MJ: scaled in pixel space
       
        renders_latent =  self.zero123plus.vae.encode(  #MJ: encoding 320x320 to 40x40 by compressing 8 times
        ...
        self.scaled_gt_renders_latent_allviews = scale_latents(renders_latent)  #MJ: scaled in latent space
        ...
        cropped_depth_rgba_grid = torch.cat((
        ...
        self.log_train_image(cropped_depth_rgba_grid[:, 0:3], 'zero123plus-prep:cropped_depth_grid')
        for i in range( len( self.thetas) ):
           self.log_train_image(self.cropped_rgb_renders_list[i], f'zero123plus-prep:v={i}: cropped_rgb_render (with default median texture)')   
        ...
        def on_step_end(pipeline, iter, t, callback_kwargs):
            ...
            #MJ: Blend the grid latent images with the ground truth render images from the mesh
            #In the case of "interleave mode", we also need to include the front view image to the blending operation
            
            if not self.cfg.guide.interleave_mode:
                # Noisify the GT rendered image at the same noise level as the latent  in the latent space
                
                
                noises_latent_otherviews = torch.randn_like( grid_latent_otherviews)
                noised_gt_renders_latent_otherviews = pipeline.scheduler.add_noise( self.scaled_gt_renders_latent_allviews[1:] , noises_latent_otherviews, t[None]) 
                ...
                unscaled_latent_otherviews = unscale_latents(blended_latents_otherviews)  #MJ: grid_latent = callback_kwargs["latents"]
                scaled_images =  pipeline.vae.decode(
                    unscaled_latent_otherviews.half() / pipeline.vae.config.scaling_factor,
                    return_dict=False
                )[0]  
                unscaled_images = unscale_image(scaled_images)
                ...
                unscaled_images_no_grad =  unscaled_images.detach()
                    
                do_denormalize = [True] * unscaled_images_no_grad.shape[0]
                postprocessed_images =  pipeline.image_processor.postprocess(unscaled_images_no_grad, output_type='pt', do_denormalize= do_denormalize) #.postprocess does not like it
                #Normalize an image array to [-1,1]. => denormalize into [0,1]
                # Now, let's save this image
                #MJ: .save() will be used only for the PIL image, not for tensor image:
                # postprocessed_image[0].save(self.train_renders_path/f'step={iter}:decoded_latent_image_in_pt.jpg')
                for i in range( len(self.thetas) - 1):
                  self.log_train_image(  postprocessed_images[i][None], f'on_step_end:denoised image-iter={iter}-v={i}') 
                ...
                # self.log_train_image( decoded_image[None], f'decoded latent image in tensor-step={iter}') 
                ...
                return callback_kwargs
                 ...
                 ##In the case of "interleave mode",  we need the latent image of the front view image; Ideally,
                 # this front view latent image at step of denoising would be generated by the SD2 depth pipeline using the text prompt with the "front view"
                 # word. But for simplicity, we simply add noise at level t to the clean front view image to obtain the latent front view image
                 # 
                
                 
                noises_latent_allviews = torch.randn_like( self.mask_allviews_latent)
                #noises_latent_front = torch.randn_like( self.scaled_front_image_latent)
                
                noised_gt_renders_latent_allviews = pipeline.scheduler.add_noise( self.scaled_gt_renders_latent_allviews , noises_latent_allviews, t[None]) 
                # #MJ: During the operation, t[None] would behave as if it is a tensor of shape (1, 1, 1, 1), effectively repeating its value 
                # # across all other dimensions to match the size of (6, 4, 40, 40).
                #self.scaled_gt_renders_latent_allviews = scale_latents(renders_latent)  #MJ: scaled in latent space
                               
                noisy_front_view_latent = noised_gt_renders_latent_allviews[0:1] 
                noisy_latent_allviews = torch.cat([ noisy_front_view_latent,  grid_latent_otherviews], dim=0)
                blended_latents_allviews = noisy_latent_allviews  * self.mask_allviews_latent +  \
                                                noised_gt_renders_latent_allviews * (1 - self.mask_allviews_latent)
                                                
                callback_kwargs["latents"] = torch.cat((
                    torch.cat((blended_latents_allviews[1][None], blended_latents_allviews[4][None]), dim=3),
                    torch.cat((blended_latents_allviews[2][None], blended_latents_allviews[5][None]), dim=3),
                    torch.cat((blended_latents_allviews[3][None], blended_latents_allviews[6][None]), dim=3),
                ), dim=2).half()
            ...
            # x = postprocess(unscale_image(vae_decode(unscale_latents(z) / scaling_factor)))
                ...
                unscaled_latent_allviews = unscale_latents(blended_latents_allviews)  #MJ: grid_latent = callback_kwargs["latents"]
                scaled_images =  pipeline.vae.decode(
                    unscaled_latent_allviews.half() / pipeline.vae.config.scaling_factor,
                    return_dict=False
                )[0]  
                unscaled_images = unscale_image(scaled_images)
                #scaled_image.requires_grad = False #MJ: somehow pipeline.vae.decode converts the tensor to the tensor with requires_grad= True
                ...
                # postprocessed_image[0].save(self.train_renders_path/f'step={iter}:decoded_latent_image_in_pt.jpg')
                for i in range( len(self.thetas)):
                  self.log_train_image(  postprocessed_images[i][None], f'pon_step_end:denoised image-iter={iter}-v={i}') 
                #MJ: unscaled_image has not been de-normalized into (0,1) by the function which returns it; The de-normalization is performed
                # #at the end of the denoising.
                # decoded_image = pipeline.image_processor.denormalize(unscaled_image_no_grad[0])
                # self.log_train_image( decoded_image[None], f'decoded latent image in tensor-step={iter}') 
                ...
                rgb_outputs_full_size = F.interpolate(postprocessed_images, (1200,1200), mode='bilinear',
                                         align_corners=False) # JA: Shape of postprocessed_image is (320,320)

                self.project_back_only_texture_atlas(   #When render_cache is not None, the camera pose is not needed
                     render_cache=self.render_cache, background=self.background_gray, rgb_output= rgb_outputs_full_size,
                     object_mask=self.object_mask, update_mask=self.object_mask, z_normals=None, z_normals_cache=None
            
                )
                
                return callback_kwargs
            #End else of  if not self.cfg.guide.interleave_mode:    
        ...
        grid_image = self.zero123plus(
            cond_image,
            depth_image=depth_image,
            num_inference_steps=self.cfg.guide.num_denoising_steps,#100,
            output_type='pt',
            #MJ: output_type ='pil" by default; To avoid the convertion back to the pixel space with (0,1) range,
            # we will use 'pt' (pytorch tensor) type in which the pixel values range in (0,1)
        ...
        #But we do that only in the case of the non-interleaving mode; In the interleaving mode, this projection back
        # has been applied within on_step_end() callback
       ...
       # The result image is already in the tensor format in which the pixel values range in (0,1)
        # So, we not use the following conversion statementx.intersection(y)
        #        
        #MJ grid_image = torchvision.transforms.functional.pil_to_tensor(result).to(self.device).float() / 255

        self.log_train_image(grid_image[None], f'zero123plus:result_grid_image:steps={self.cfg.guide.num_denoising_steps}')


        print(f"Elapsed time in zero123 sampling  in non-interleave mode or interleaving mode: {elapsed_time} seconds")
        
        if not self.cfg.guide.interleave_mode:              
        
        
            project_back_final_start_time =  time.perf_counter()  # Record the end time
                
            
            tile_size = 320
            grid_images_otherviews = utils.split_3x2_grid_to_tensor_with_6_elements(grid_image[None], tile_size) 
            #MJ: images = split_zero123plus_grid(grid_image[None], 320) #MJ: grid_image: shape =(3,960,640)
            ...
            for i in range (len(self.thetas) ):
                if i == 0:
                    ...
                    rgb_output_small =  grid_images_otherviews[i-1][None]
                    # image_row_index = (i - 1) % 3
                    # image_col_index = (i - 1) // 3

                    # rgb_output_small = images[image_row_index][image_col_index]
                ...
                #End else:
                rgb_outputs_list.append(rgb_output)
            #End for i in range (len(self.thetas) ):
            
            #MJ: Project-back the generated view images, rgb_outputs, to the texture atlas.
            rgb_outputs =  torch.cat(rgb_outputs_list, dim=0)
            
           
            
            project_back_start_time =  time.perf_counter()  # Record the end time
            #MJ: for debugging
            for i in range(rgb_outputs.shape[0]):
              self.log_train_image(rgb_outputs[i][None] * self.object_mask[i][None], f'project_back_input_{i}')  
        
            self.project_back_only_texture_atlas(   #When render_cache is not None, the camera pose is not needed
                render_cache=self.render_cache, background=self.background_gray, rgb_output= rgb_outputs,
                object_mask=self.object_mask, update_mask=self.object_mask, z_normals=None, z_normals_cache=None
            ...
            project_back_final_end_time =  time.perf_counter()  # Record the end time 
            
            elapsed_time =  project_back_final_end_time -  project_back_final_start_time
            print(f'project-back_final  time={elapsed_time:0.4f}')
            
        #End if not self.cfg.guide.interleave_mode    
    ...
    def evaluate(self, dataloader: DataLoader, save_path: Path, save_as_video: bool = False): #MJ: dataloader=self.dataloaders['val']
        ...
        if save_as_video:
            ...
            dump_vid = lambda video, name: imageio.mimsave(save_path / f"eval:constructed_video:{name}_steps={self.cfg.guide.num_denoising_steps}_alpha:{self.cfg.optim.alpha}.mp4", video,
                                                           fps=25,
    ...
    def paint_viewpoint(self, data: Dict[str, Any], should_project_back=True):
        ...
        self.log_train_image(cropped_rgb_render, name='paint_viewpoint:cropped_rgb_render(with the default magenta tex (for the frontview))')
        ...
        self.log_train_image(cropped_rgb_output, name='paint_viewpoint:cropped_rgb_output (blended output)')
        ...
        self.log_train_image(rgb_output, name='paint_viewpoint:rgb_render_full (blended: why mangenta boundary??)')
        ...
        if should_project_back:
            ...
            self.log_train_image(fitted_pred_rgb, name='paint_viewpoint:fitted_pred_rgb (shows the quality of the texture learned from the front view)')
        ...
        return fitted_pred_rgb, object_mask
        #return rgb_output, object_mask #MJ: fitted_rgb_rgb is a full screen image as is  rgb_output
    ...
    def project_back(self, render_cache: Dict[str, Any], background: Any, rgb_output: torch.Tensor,
        ...
        for i in range(rgb_output.shape[0]):
            self.log_train_image(rgb_output[i][None] * render_update_mask[i][None], f'project_back:project_back_input_{i}')
        ...
        with  tqdm(range(300), desc='project_back (SD2): fitting mesh colors for the front view') as pbar:
          for i in pbar:  #MJ: Here we do not have the batch loop, but only the epoch loop
            ...
            optimizer.step()  #+> self.texture_img is adjusted, that is, learned
        ...
        # #MJ:  rgb_render (which will become rgb_output_front) has the magenta default color of the texture along the boundary of the rendered front view image
        # # This affects badly  the project_back_only_texture_atlas() which learns the texture map from the generated view images.
        # #MJ: modify  rgb_output_front a little bit by changing  the default magenta region of the texture atlas
        # #  to the median color of the learned texture map, and  re-rendering the updated texture map
        # if  self.cfg.guide.use_zero123plus:   
        #     diff = (self.mesh_model.texture_img - torch.tensor(self.mesh_model.default_color).view(1, 3, 1, 1).to(
        #         self.device)).abs().sum(axis=1)
        #     default_mask = (diff < 0.1).float().unsqueeze(0)
        #     median_color = self.mesh_model.texture_img[0, :].reshape(3, -1)[:, default_mask.flatten() == 0].mean(
        #         axis=1)
        #     #MJ: texture_img = self.mesh_model.texture_img.clone()
        #     with torch.no_grad():
        #         self.mesh_model.texture_img.reshape(3, -1)[:, default_mask.flatten() == 1] = median_color.reshape(-1, 1)
        #     #MJ: re-render the mesh using the updated texture map from all views    
        #     outputs = self.mesh_model.render(background=background, #MJ:  render_caches contains the info about all viewpoint projection and rasterization data
        #                                      render_cache=render_cache)    
    ...
    #MJ:  Effect of torch.no_grad(): By using torch.no_grad(), any modifications to tensors will not be tracked, and hence no gradients will be computed or stored for these operations.
    # This is typically used during inference or when applying manual updates to tensors that should not affect the gradient flow 
    # (like updating non-trainable model parameters or applying fixed transformations to data).
    # Thus, while the modifications made within the torch.no_grad() block don't accumulate gradients themselves, 
    # any further operations on these modified values (once outside the torch.no_grad() block) will influence gradient computations.
