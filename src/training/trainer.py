...
class TEXTure:
    ...
    def paint_zero123plus(self):
        ...
        self.outputs = self.mesh_model.render(theta=self.thetas, phi=self.phis, radius=self.radii,
       ...
       # but got input with spatial dimensions of [3, 886, 886] and output size of (320, 320)               
        ...
        cropped_front_image_small = F.interpolate(
                            cropped_front_image, (320, 320),   
                            mode='bilinear',
                            align_corners=False
            )
        ...
        preprocessed_cropped_front_image_small = self.zero123plus.image_processor.preprocess(cropped_front_image_small)
           
        scaled_rgb_renders_small =  scale_image(preprocessed_rgb_renders_small.half() ) #MJ: scaled in pixel space
        scaled_front_image_small =  scale_image(preprocessed_cropped_front_image_small.half() )
        ...
        front_image_latent =  self.zero123plus.vae.encode(  #MJ: encoding 320x320 to 40x40 by compressing 8 times
                  scaled_front_image_small,
               return_dict=False
            )[0].sample() * self.zero123plus.vae.config.scaling_factor 

        self.scaled_gt_renders_latent_allviews = scale_latents(renders_latent)  #MJ: scaled in latent space
        self.scaled_front_image_latent = scale_latents(front_image_latent)
        ...
        def on_step_end(pipeline, iter, t, callback_kwargs):
            ...
            if not self.cfg.guide.interleave_mode:
                ...
                # # across all other dimensions to match the size of (6, 4, 40, 40).
                #self.scaled_gt_renders_latent_allviews = scale_latents(renders_latent)  #MJ: scaled in latent space
                noisy_front_image_latent = pipeline.scheduler.add_noise( self.scaled_front_image_latent,  noises_latent_front, t[None]) 
                
                #MJ: debug:
                #It is supposed that self.scaled_front_image_latent is the same as self.scaled_gt_renders_latent_allviews[0]
                print(f'being_close(self.scaled_front_image_latent, self.scaled_gt_renders_latent_allviews[0]) \
                        ={torch.allclose(self.scaled_front_image_latent,self.scaled_gt_renders_latent_allviews[0][None],  atol=1e-3, rtol=1e-3) }')
                 
                 
                noisy_latent_allviews = torch.cat([noisy_front_image_latent,  grid_latent_otherviews], dim=0)
            ...
            # x = postprocess(unscale_image(vae_decode(unscale_latents(z) / scaling_factor)))
                ...
                for i in range( len(self.thetas)):
                  self.log_train_image(  postprocessed_images[i][None], f'postprocessed imag in pt tensor-step={iter}-v={i}') 
