...
class TEXTure:
    ...
    def paint_zero123plus(self):
        ...
        # #MJ: cropped_rgb_renders_list = self.get_cropped_rgb_renders(rgb_renders_magenta, object_masks)
        # masks_latent_list = []
        ...
        #     #MJ: added for debugging: do not use diff-based mask but the simple object-masks
        #     cropped_generate_mask = object_masks[i][None]
        ...
        #     #MJ: for debugging, do not use dilating the boundary of the mask
        #     dilated_cropped_mask =   cropped_generate_mask
        ...
        for i in range(B):
            ...
            cropped_rgb_render =  crop( rgb_renders_magenta[i][None])
            ...
            cropped_masks_latent_list.append( F.interpolate(
                  cropped_mask, (320//8, 320//8),   
                mode='bilinear',
                align_corners=False
            ))
        ...
        masks_grid_latent = torch.cat((
            torch.cat((cropped_masks_latent_list[1], cropped_masks_latent_list[4]), dim=3),
            torch.cat((cropped_masks_latent_list[2], cropped_masks_latent_list[5]), dim=3),
            torch.cat((cropped_masks_latent_list[3], cropped_masks_latent_list[6]), dim=3),
        ...
        self.log_train_image(cropped_rgb_renders_grid, 'zero123plus-prep:cropped_rgb_renders_grid')
    ...
    def project_back(self, render_cache: Dict[str, Any], background: Any, rgb_output: torch.Tensor,
        ...
        #MJ: use the object mask as the render update mask (which is obtained by blurring the boundary)
