...
class StableDiffusion(nn.Module):
    def __init__(self, device, model_name='CompVis/stable-diffusion-v1-4', concept_name=None, concept_path=None,
        ...
        # 3. The UNet models for generating the latents.
        ...
        # 4. Create a scheduler for inference: MJ: The same scheduler is used for self.unet and self.inapint_unet, and the zero123 unet net
...
#MJ:  cond, uc = self.get_zero123_inputs(
    ...
    def get_zero123_inputs(
        ...
        c = self.second_model.get_learned_conditioning(cond_image).tile(n_samples, 1, 1) #MJ: c: shape =(1,1,768)
        ...
        T = T[None, None, :].repeat(n_samples, 1, 1).to(c.device) #MJ: T: shape = [1,1,4]
        c = torch.cat([c, T], dim=-1)                              #MJ: c: shape =[1,1,768]  
        c = self.second_model.cc_projection(c)
        cond = {}
        cond['c_crossattn'] = [c]
        cond['c_concat'] = [self.second_model.encode_first_stage((cond_image.to(c.device))).mode().detach().repeat(n_samples, 1, 1, 1)] #MJ: concat: 64x64
    ...
    def img2img_step(self, text_embeddings, inputs, original_depth_mask, i, t, timesteps, guidance_scale=100,
        ...
        def sample(latents, i, t, timesteps, depth_mask, update_mask=None, check_mask=None,
            ...
            if latents is None:
                ...
                if fixed_seed is not None: # JA: In the case of the first view, we do not use the phi/theta angle, but the depth mask reflects the viewpoint information
                    seed_everything(fixed_seed) # JA: In our old version, seed_everything was called for each viewpoint
                noise = torch.randn_like(latents) # JA: Every time torch.randn_like(latents) is executed, the new random tensor, noise, is created
        ...
        with torch.no_grad():
            ...
            # target_latents = sample(latents, i, t, depth_mask, strength=strength, num_inference_steps=num_inference_steps,
            target_latents = sample(latents, i, t, timesteps, depth_mask,
