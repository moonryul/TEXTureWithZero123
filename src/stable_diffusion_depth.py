        ...
        def sample(latents, depth_mask, strength, num_inference_steps, update_mask=None, check_mask=None,
            ...
            with torch.autocast('cuda'):
                for i, t in tqdm(enumerate(timesteps)): # JA: denoising iteration loop of the sample function
                    ...
                    #MJ: Performing the blending creates the magenta boundary on the rendered image
                    # because the original default magenta color is put into the ground truth render image
                    # Disable it.
                    #MJ: While not performing inpainting, perform blending before the execution of the denoising step
                    # either by the depth pipeline or the inpaint pipeline
                    if not is_inpaint_range and self.use_blending: 
                        if update_mask is not None:
                            ...
                            #latents = latents * curr_mask + gt_latents *(1 - curr_mask)
                    # Execute  the denoising step  either by the depth pipeline or the inpaint pipeline
                    ...
                    #At this point, i == len(timesteps) - 1, t =0, and we have reached the last iteration 
                    # of denoising loop. Blend this last latent with the clean ground truth render image
                    # 
                    #   
                    if i == len(timesteps) -1 and self.use_blending:
                         latents = latents * curr_mask + gt_latents * (1 - curr_mask) 
                         
                #End for i, t in tqdm(enumerate(timesteps))
